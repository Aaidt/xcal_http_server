...
if (cluster.isPrimary) {
   ...
   console.log(`Primary process: ${process.pid} has started...`);
   ...
   cluster.on("exit", (worker) => {
      console.log(`Worker process ${worker.process.pid} died. Creating a new one...`);
      cluster.fork();
   })
} else {
   ...
   const server = app.listen(3002, () => { console.log("Server is listening on port 3002") });


   process.on("SIGTERM", async () => {
      console.log(`worker thread ${process.pid} is shutting down...`)
